...
local g_UnitFlagClass = 
{
    ...
    UpdateSelected = function( self, isSelected )
        ...
        if pUnit then
            self.m_IsInGroup = pUnit:IsHasPromotion(GameInfo.UnitPromotions["PROMOTION_GROUP"].ID) or pUnit:IsHasPromotion(GameInfo.UnitPromotions["PROMOTION_GROUP_AIR"].ID);
            bGUISTIsHide = (not self.m_IsInGroup and (self.m_StackOrders[1] <= 1 or not (pUnit:CanMove() or pUnit:CanDoCommand(CommandTypes.COMMAND_CANCEL)))) or g_IsDoingGG or not Players[Game.GetActivePlayer()]:IsTurnActive() or (self.m_Escort and self.m_Escort:IsCity());
            bGGBoxIsHide =  not pUnit:IsFriendlyUnitAdjacent() or not (self.m_IsInGroup or pUnit:CanMove() or pUnit:CanDoCommand(CommandTypes.COMMAND_CANCEL)) or g_IsDoingGG or not Players[Game.GetActivePlayer()]:IsTurnActive() or (self.m_Escort and self.m_Escort:IsCity());
        end
...
function UpdateCargoList()
	...
	if( pPlot:IsCity() ) then
		...
		for i = 0, pPlot:GetNumUnits() - 1 do
			...
			if pAirUnit:GetDomainType() == DomainTypes.DOMAIN_AIR and not pAirUnit:IsInvisible(Game.GetActiveTeam()) then
				...
				if( pAirUnit:GetOwner() ~= Game.GetActivePlayer() ) then
					...
					afCount = afCount + 1;
					controlTable.GroupMark:RegisterCallback( Mouse.eLClick, function() OnGroupSetup( _, pAirUnit, 11, false ); end );
					...
					gmCount = gmCount + 1;
					controlTable.GroupMark:RegisterCallback( Mouse.eLClick, function() OnGroupSetup( _, pAirUnit, 12, false ); end );
					if isHasPromotion then
						controlTable.GroupMark:SetText( "[ICON_BULLET]" );
						controlTable.GroupMark:LocalizeAndSetToolTip( "TXT_KEY_AIRCRAFT_OFF_TT" );
					else
		...
		for i = 0, pPlot:GetNumUnits() - 1, 1 do
			...
			if ( pCarrierUnit:HasCargo() and not pCarrierUnit:IsInvisible(Game.GetActiveTeam()) ) then
				...
				for j = 0, pPlot:GetNumUnits() - 1 do
					...
					then
						...
						if( pCargoUnit:GetOwner() ~= Game.GetActivePlayer() ) then
							...
							afCount = afCount + 1;
							cargoCoTable.GroupMark:RegisterCallback( Mouse.eLClick, function() OnGroupSetup( _, pCargoUnit, 11, false ); end );
							if isHasPromotion then
							...
							gmCount = gmCount + 1;
							cargoCoTable.GroupMark:RegisterCallback( Mouse.eLClick, function() OnGroupSetup( _, pCargoUnit, 12, false ); end );
							if isHasPromotion then
								cargoCoTable.GroupMark:SetText( "[ICON_BULLET]" );
								cargoCoTable.GroupMark:LocalizeAndSetToolTip( "TXT_KEY_AIRCRAFT_OFF_TT" );
							else
				...
				controlTable.GroupMark:RegisterCallback( Mouse.eLClick, function() OnGroupSetup( not cargoIsAllCheckSet, pCarrierUnit, 13, true ); end );
		...
		for k = 0, pPlot:GetNumUnits() - 1, 1 do
			...
			if not ( pOtherUnit:HasCargo() or pOtherUnit:IsCargo() ) then
				...
				TruncateString( controlTable.CarriName, 155, Locale.ConvertTextKey( "[ICON_MOVES]" .. pOtherUnit:GetName() ) );
	...
	if     afCount > 0 and  gmCount > 0 then
		...
		g_UnitList.T2Check:RegisterCallback( Mouse.eLClick, function() OnGroupSetup( not aFIsAllCheckSet, pPlot:GetUnit( 0 ), 11, true ); end );
		g_UnitList.T1Check:RegisterCallback( Mouse.eLClick, function() OnGroupSetup( not gMIsAllCheckSet, pPlot:GetUnit( 0 ), 12, true ); end );
		if aFIsAllCheckSet then
	...
	elseif afCount > 0 then
		...
		g_UnitList.T1Check:RegisterCallback( Mouse.eLClick, function() OnGroupSetup( not aFIsAllCheckSet, pPlot:GetUnit( 0 ), 11, true ); end );
		if aFIsAllCheckSet then
...
function OnUISTClicked( playerID, unitID )
	...
	-- Set GGOption
	local IsCheckSet = pUnit:IsHasPromotion(GameInfo.UnitPromotions["PROMOTION_GROUP"].ID) or pUnit:IsHasPromotion(GameInfo.UnitPromotions["PROMOTION_GROUP_AIR"].ID);
						...
						for i = 0, adjPlot:GetNumUnits() - 1 do
							...
							then
								...
								then
									...
									adjLMPlot = Map.PlotDirection(pLastMissionPlot:GetX(), pLastMissionPlot:GetY(), direction);
									if adjLMPlot ~= nil then
										if not adjUnit:CanMoveThrough(adjLMPlot) then
											local tempAdjLMPlot = nil;
											for directionLM = 0, 5 do
												tempAdjLMPlot = Map.PlotDirection(adjLMPlot:GetX(), adjLMPlot:GetY(), directionLM);
									...
									if adjLMPlot == nil then
										local tempAdjLMPlot = nil;
										for directionLM = 0, 5 do
											tempAdjLMPlot = Map.PlotDirection(pLastMissionPlot:GetX(), pLastMissionPlot:GetY(), directionLM);
											if tempAdjLMPlot and adjUnit:CanMoveThrough(tempAdjLMPlot) then
												break;
											end
											-- not Plot to Move into!
...
function CheckPlot( plot )
	...
	if ( plot:GetNumLayerUnits() > 1 ) then
		...
		if( g_PrintDebug ) then print( string.format( "Determining Plot for: Player[%i] Unit[%i] - %i", self.m_PlayerID, self.m_UnitID, i ) ); end
...
function TipHandler( Button )
	...
	if Players[iPlayer] and Players[iPlayer]:GetUnitByID(iUnit) then
		...
		if unit.IsInOrbit and unit:IsInOrbit() then
			toolTipString = toolTipString .. " " .. "[COLOR_CYAN]" .. Locale.ConvertTextKey( "TXT_KEY_PLOTROLL_ORBITING" )  .. "[ENDCOLOR]"
			...
			if unit.GetReligion then
				...
				if religionID > 0 then
					...
					toolTipString = toolTipString .. ((GameInfo.Religions[ religionID ] or {}).IconString or "?") .. Locale.ConvertTextKey( Game.GetReligionName( religionID ) )
				end
			end
		...
		if iPlayer == activePlayerID then
			...
			if player:GetUnitClassCount( unitClassID ) == 0 and player:GetUnitClassMaking( unitClassID ) == 0 then
				...
				if player:GetUnitClassCount( unitClassID ) > 0 then
					unitClassText = "[ICON_BULLET]" .. Locale.ConvertTextKey( "TXT_KEY_ACTION_CLASS_BUILT_COUNT", player:GetUnitClassCount( unitClassID ) );
					if player:GetUnitClassMaking( unitClassID ) > 0 then
						unitClassText = unitClassText .. " <> "  .. Locale.ConvertTextKey( "TXT_KEY_ACTION_CLASS_BUILDING_COUNT", player:GetUnitClassMaking( unitClassID ) );
					end
				else
					unitClassText = "[ICON_BULLET]" .. Locale.ConvertTextKey( "TXT_KEY_ACTION_CLASS_BUILDING_COUNT", player:GetUnitClassMaking( unitClassID ) );
				end
			end
		end
