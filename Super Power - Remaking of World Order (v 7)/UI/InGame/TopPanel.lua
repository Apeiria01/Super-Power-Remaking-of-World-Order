		...
		if (pPlayer:GetNumCities() > 0) then
			...
			for pResource in GameInfo.Resources() do
				...
				if (Game.GetResourceUsageType(iResourceLoop) == ResourceUsageTypes.RESOURCEUSAGE_STRATEGIC) then
					...
					if (pResource.TechReveal == nil and pResource.TechCityTrade == nil)
					or (pTeam:GetTeamTechs():HasTech(GameInfoTypes[pResource.TechReveal])
					and pTeam:GetTeamTechs():HasTech(GameInfoTypes[pResource.TechCityTrade]))
					then
						bShowResource = true;
					end
					...
					-- Must be one to show
					if (iMin == nil or iMin > iNumAvailable) and pResource.Type ~= "RESOURCE_TROOPS" and bShowResource then
						iMin = iNumAvailable;
					...
					if (bShowResource) then
						strTempText = string.format("%i%s ", iNumAvailable, pResource.IconString);
			...
			if strResourceText == "" then
				strResourceText = strMinText;
			end
			strResourceText = strResourceText .. strTroopText;
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_POLICIES)) then
		...
		if (not OptionsManager.IsNoBasicHelp()) then
			...
			if pPlayer:HasPolicy(GameInfo.Policies["POLICY_REPRESENTATION"].ID) then
				strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_CULTURE_CITY_COST_MOD");
			else
