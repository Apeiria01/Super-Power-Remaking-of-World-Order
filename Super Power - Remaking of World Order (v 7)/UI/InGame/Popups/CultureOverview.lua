...
function RegisterSortOptions()
    ...
    for i, v in ipairs(g_YourCultureSortOptions) do
        if (v.Button ~= nil) then
            v.Button:RegisterCallback(Mouse.eLClick, function()
                YourCultureSortOptionSelected(v);
            end);
        end
    end
    ...
    for i, v in ipairs(g_CultureVictorySortOptions) do
        if (v.Button ~= nil) then
            v.Button:RegisterCallback(Mouse.eLClick, function()
                CultureVictorySortOptionSelected(v);
            end);
        end
    end
    ...
    for i, v in ipairs(g_InfluenceByPlayerSortOptions) do
        if (v.Button ~= nil) then
            v.Button:RegisterCallback(Mouse.eLClick, function()
                InfluenceByPlayerSortOptionSelected(v);
            end);
        end
    end
...
Controls.TabButtonYourCulture:RegisterCallback(Mouse.eLClick, function()
    TabSelect("YourCulture");
end);
Controls.TabButtonSwapGreatWorks:RegisterCallback(Mouse.eLClick, function()
    TabSelect("SwapGreatWorks");
end);
Controls.TabButtonCultureVictory:RegisterCallback(Mouse.eLClick, function()
    TabSelect("CultureVictory");
end);
Controls.TabButtonPlayerInfluence:RegisterCallback(Mouse.eLClick, function()
    TabSelect("PlayerInfluence");
end);
...
function getBuildings()
    local gwBuildings = {};
    local gwBuildingsAdd = {};

    -- Get all default, non-Wonder buildings with at least 1 Great Work slot.
    for row in DB.Query([[
		...
		EXISTS (
			...
			WHERE	MaxPlayerInstances = -1 AND
				...
				DefaultBuilding = b.Type	-- CivilizationType IS NULL
	]]) do
    ...
    for row in DB.Query([[
        ...
        if gwBuildings[row.BuildingClass] then
            if row.GreatWorkCount <= 0 then
                gwBuildingsAdd[row.BuildingClass] = gwBuildings[row.BuildingClass];
                gwBuildings[row.BuildingClass] = nil;
            else
                gwBuildings[row.BuildingClass] = row;
            end
            dprint(row.BuildingType);
        elseif row.GreatWorkCount > 0 then
            gwBuildingsAdd[row.BuildingClass] = row;
            dprint(row.BuildingType);
        end
    end

    -- find overrides with Great Work slot(s) from other Civilizations while their default buildings without the slot(s).
    for row in DB.Query([[
		SELECT b.Id AS Id, bc.Type AS BuildingClass, b.Type AS BuildingType, GreatWorkCount, 
		CivilizationType, t.GridX, t.GridY, 
		EXISTS (
        ...
        if gwBuildings[row.BuildingClass] == nil and gwBuildingsAdd[row.BuildingClass] == nil then
            gwBuildingsAdd[row.BuildingClass] = row;
            dprint(row.BuildingType);
        end
    end
    ...
    for _, building in pairs(gwBuildingsAdd) do
        table.insert(sortedadd, building);
    end

    table.sort(sorted, buildingSort);
    table.sort(sortedadd, buildingSort);
    for _, building in pairs(sortedadd) do
        table.insert(sorted, building);
    end
...
function RefreshYourCulture()
    ...
    function BuildingEntry(data)
        ...
        if (data.BuildingClass ~= nil) then
            ...
            local override = GameInfo.Civilization_BuildingClassOverrides {
                BuildingClassType = data.BuildingClass,
                CivilizationType = playerCivilizationType
            }();
    ...
    function AssignBuildingToColumn(building, buildings)
        ...
        if (building.GreatWorkCount > 1 and building.HasThemeBonusInt == 0) then
            estimatedColumnSize = estimatedColumnSize - 23;
        end
    ...
    for building in GameInfo.Buildings() do
        ...
        -- Kyte
        if (building.GreatWorkCount > 0 and not BuildingClassIsAColumn(buildingClass.Type, buildings)) then
...
function SortAndDisplayYourCulture()
    ...
    function GetHighlightTexture(data)
        ...
        if (currentGreatWorkSlot ~= nil and data.GreatWorkSlotType == currentGreatWorkSlot.GreatWorkSlotType) then
            if (currentGreatWorkSlot.CityID == data.CityID and currentGreatWorkSlot.BuildingClassID == data.BuildingClassID and currentGreatWorkSlot.GreatWorkSlotIndex == data.GreatWorkSlotIndex) then
        ...
        for buildingIndex, buildingData in ipairs(cityData.Buildings) do
            ...
            if (buildingData.HasBuilding) then
                ...
                for greatWorkSlotIndex = 1, math.min(buildingData.GreatWorkSlotCount, #greatWorkIconControls), 1 do
                    ...
                    if (previousGreatWorkSelection ~= nil and previousGreatWorkSelection.CityID == selectionPoint.CityID and previousGreatWorkSelection.BuildingClassID == selectionPoint.BuildingClassID and previousGreatWorkSelection.GreatWorkSlotIndex == selectionPoint.GreatWorkSlotIndex) then
        ...
        if (g_YourCultureSelectionMode == "ClickToView") then
            ...
            selectionPoint.Control:RegisterCallback(Mouse.eLClick, function()
                if (g_CurrentGreatWorkSlot and (g_CurrentGreatWorkSlot.GreatWorkSlotType == selectionPoint.GreatWorkSlotType)) then
                    ...
                    local currentGreatWorkSlot = g_CurrentGreatWorkSlot;
                    if (currentGreatWorkSlot.CityID ~= selectionPoint.CityID or currentGreatWorkSlot.BuildingClassID ~= selectionPoint.BuildingClassID or currentGreatWorkSlot.GreatWorkSlotIndex ~= selectionPoint.GreatWorkSlotIndex) then
    ...
    for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS - 1 do
        ...
        if (pPlayer:IsAlive() and not pPlayer:IsMinorCiv() and activeTeam:IsHasMet(pPlayer:GetTeam())) then
            ...
            if (iIdeology ~= PolicyBranchTypes.NO_POLICY_BRANCH_TYPE) then
                ...
                row.strPublicOpinionUnhappiness = strPublicOpinionUnhappiness .. " / " .. strExcessHappiness;
            ...
            if pPlayer ~= activePlayer and iImmigrationRegressand > 0 then
				...
				elseif ImmigrationRate < 0 then
					strImmigrationRate = Locale.ConvertTextKey("TXT_KEY_CO_SP_RED", ImmigrationRate);
				...
				local iActiveExcessHappiness = activePlayer:GetExcessHappiness();
				local iPExcessHappiness = pPlayer:GetExcessHappiness();
				if iActiveExcessHappiness >= 150 then
				...
				elseif iActiveExcessHappiness >= 100 then
				...
				elseif iActiveExcessHappiness >= 50 then
					
				elseif iActiveExcessHappiness >= 20 then
				...
				elseif iActiveExcessHappiness >= 0 then
				...
				elseif iPExcessHappiness > iActiveExcessHappiness and pPlayer:HasTrait(GameInfoTypes["TRAIT_RIVER_EXPANSION"]) then
            ...
            ---------------------------- SP International Immigration END -----------------------------------------------
...
Controls.VictoryPlayerNameStack:SetOffsetX(8);
Controls.IdeologyHeader:SetSizeX(116);
Controls.PublicOpinionHeader:SetSizeX(148);
---------------------------- SP International Immigration HeaderChange ---------------------------------------------------
if Game.GetImmigrationRegressand() > 0 then
    Controls.PublicOpinionUnhappinessHeader:LocalizeAndSetToolTip("TXT_KEY_CO_VICTORY_PUBLIC_OPINION_UNHAPPINESS_EXCESS_HAPPINESS_HEADER_TT");
    Controls.ExcessHappinessHeader:LocalizeAndSetToolTip("TXT_KEY_CO_IMMIGRATION_COUNTER_HEADER_TT");
    Controls.POUHeaderLabel:LocalizeAndSetText("TXT_KEY_CO_VICTORY_PUBLIC_OPINION_UNHAPPINESS_EXCESS_HAPPINESS_HEADER");
    Controls.EHHeaderLabel:LocalizeAndSetText("TXT_KEY_CO_IMMIGRATION_COUNTER_HEADER");
end
---------------------------- SP International Immigration HeaderChange END -----------------------------------------------
        ...
        if (pPlayer:IsAlive() and not pPlayer:IsMinorCiv()) then
            ...
            if (iPlayer ~= g_iSelectedPlayerID) then
                ...
                if (iInfluenceLevel ~= InfluenceLevelTypes.NO_INFLUENCE_LEVEL) then
                    ...
                    if Game.GetImmigrationRegressand() > 0 and iInfluenceLevel > 0 then
                        playerInfluence.LevelToolTip = playerInfluence.LevelToolTip .. Locale.ConvertTextKey("TXT_KEY_CO_INFLUENCE_BONUSES_IMMIGRATION", iInfluenceLevel);
                    end
...
function SortAndDisplayPlayerInfluence()
    ...
    for i, playerInfluence in ipairs(model.PlayerInfluence) do
        ...
        if (trendRate == -1) then
            ...
            instance.InfluenceTrendIcon:SetColor{
                x = 1,
                y = 0,
                z = 0,
                w = 1
