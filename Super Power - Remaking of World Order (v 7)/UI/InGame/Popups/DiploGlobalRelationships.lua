			...
			if (g_pUsTeam:IsHasMet(iOtherTeam)) then
				...
				for pBuilding in GameInfo.Buildings() do
					...
					if (pBuildingClass.MaxGlobalInstances > 0) then
						if (pOtherPlayer:CountNumBuildings(pBuilding.ID) > 0) then
							local textControls = {};
                ...
                -- Projects
				local bHasProject = false
                for iProject in GameInfo.Projects() do
                    if pOtherTeam:GetProjectCount(iProject.ID) > 0
                    then
                        if iProject.MaxGlobalInstances == 1 or iProject.MaxTeamInstances == 1 then
                            local textControls = {};
							ContextPtr:BuildInstanceForControl("LTextEntry", textControls, controlTable.WondersStack);
							if bHasProject == false then
								textControls.Text:LocalizeAndSetText("[NEWLINE]" .. Locale.ConvertTextKey(iProject.Description));
								bHasProject = true
							else
								textControls.Text:LocalizeAndSetText(iProject.Description);
							end
                        end
                    end
                end
					...
					if (iThirdPlayer ~= iOtherPlayer) then
						...
						if (pThirdPlayer ~= nil and pThirdPlayer:IsAlive()) then
							...
							if (g_pUsTeam:IsHasMet(iThirdTeam) or iThirdPlayer == g_iUs) then
								...
								-- Research Agreements
								if pOtherTeam:IsHasResearchAgreement(iThirdTeam) then
									bHasEntry = true;
                                    ...
                                    local text = Locale.Lookup("TXT_KEY_DO_RESEARCH_AGREEMENT_WITH", thirdName);
                                    local turnsLeft = iRemainingTurn + pOtherTeam:GetResearchAgreementStartTurn(iThirdTeam);
                                    text = text .. " (" .. Locale.Lookup("TXT_KEY_DECLARE_WAR_DEALS_TURNS_LEFT", turnsLeft) .. ")";
                                    textControls.Text:SetText(text);	
								end
							end
