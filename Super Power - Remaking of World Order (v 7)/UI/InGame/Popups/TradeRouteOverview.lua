...
g_SortOptions = {
	...
	--------------SP fix sort error
	{
		...
		Column = "FromCityName",
		SecondaryColumn = "TurnsLeft",
		SecondaryDirection = "asc",
		DefaultDirection = "asc",
		CurrentDirection = "asc",
	},
		--------------SP fix sort error END
...
---------------SP end trade early will remove the unit END
	    end
	 end  
  end
...
function DisplayData()
	...
	--------------SP cancel Trade	
		...
		local strTurnsLest_TT = " "
		if (v.IsRecalled) then
		  strTurnsRemaining = "[ICON_ARROW_LEFT]"

--		  instance.TurnsLeft:SetText("[ICON_ARROW_LEFT]");
		  strTurnsLest_TT = Locale.Lookup("TXT_KEY_TRO_RECALLED");
		else
		   if (v.TurnsLeft ~= nil and v.TurnsLeft >= 0) then

 				strTurnsRemaining = "[ICON_CITYBANNER_CANCEL]";
		  		strTurnsLest_TT = Locale.Lookup("TXT_KEY_TRO_LOCATE_RECALL_TT");
		   end
		end
		instance.TurnsLeft:SetText(strTurnsRemaining);
		instance.TurnsLeft:SetToolTipString (strTurnsLest_TT);
