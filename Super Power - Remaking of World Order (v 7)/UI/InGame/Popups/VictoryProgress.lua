...
local g_TechList = {};
local g_TechProj = GameInfo.Projects[GameDefines["SPACE_RACE_TRIGGER_PROJECT"]];
...
----------------------------------------------------------------
local UAPlotSetList;
function OnTNLUCConqueror(TraitUAPlotSet)
	UAPlotSetList = TraitUAPlotSet;
end
LuaEvents.TNLUCConqueror.Add(OnTNLUCConqueror)

function PopulateDomination()
		
	if(PreGame.IsVictory(GameInfo.Victories["VICTORY_DOMINATION"].ID))
	or(PreGame.GetGameOption("GAMEOPTION_TNL_UCCONQUEROR") == 1 and UAPlotSetList and #UAPlotSetList > 0)
	then
		...
		if PreGame.GetGameOption("GAMEOPTION_TNL_UCCONQUEROR") == 1 and UAPlotSetList and #UAPlotSetList > 0 then
			for iCivType, PlotList in pairs( UAPlotSetList ) do
			    if PlotList[1] then
				...
				g_DominationItemList[numItems] = curItemIM:GetInstance();
				local civinfo = GameInfo.Civilizations[iCivType];
				local sCivName = civinfo.ShortDescription
				if PlotList[1]:IsRevealed(Game.GetActiveTeam()) then
					IconHookup( civinfo.PortraitIndex, 64, civinfo.IconAtlas, g_DominationItemList[numItems].CivIconBG );
					g_DominationItemList[numItems].CivIconShadow:SetHide( true );
					g_DominationItemList[numItems].Icon:SetHide( true );
				else
					sCivName = "TXT_KEY_RO_WR_UNKNOWN_CIV"
					CivIconHookup(-1, 64, g_DominationItemList[numItems].Icon, g_DominationItemList[numItems].CivIconBG, g_DominationItemList[numItems].CivIconShadow, false, true );
				end
				
				-- Set the Conquer Icon
				local iDominatingPlayer = PlotList[1]:GetOwner();
				if iDominatingPlayer >= GameDefines.MAX_MAJOR_CIVS then
					iDominatingPlayer = Players[iDominatingPlayer]:GetAlly();
				end
				if iDominatingPlayer >= 0 then
					if aiCapitalOwner[iDominatingPlayer] == nil then
						aiCapitalOwner[iDominatingPlayer] = 0;
					end
					aiCapitalOwner[iDominatingPlayer] = aiCapitalOwner[iDominatingPlayer] + 1;
				end
				if     iDominatingPlayer == -1 or (iDominatingPlayer >= GameDefines.MAX_MAJOR_CIVS and Players[iDominatingPlayer]:GetAlly() == -1) then
					g_DominationItemList[numItems].IconFrame:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_VP_CONTROL_CIVILIZATION", sCivName, "TXT_KEY_MISC_UNKNOWN"));
				elseif iDominatingPlayer == Game.GetActivePlayer() then
					g_DominationItemList[numItems].IconFrame:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_VP_CONTROL_CIVILIZATION", sCivName, "TXT_KEY_YOU"));
				elseif Teams[Players[iDominatingPlayer]:GetTeam()]:IsHasMet(Game.GetActiveTeam()) then
					g_DominationItemList[numItems].IconFrame:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_VP_CONTROL_CIVILIZATION", sCivName, GameInfo.Leaders[Players[iDominatingPlayer]:GetLeaderType()].Description));
				else
					g_DominationItemList[numItems].IconFrame:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_VP_CONTROL_CIVILIZATION", sCivName, "TXT_KEY_UNMET_PLAYER"));
					iDominatingPlayer = -1;
				end
				CivIconHookup(iDominatingPlayer, 45, g_DominationItemList[numItems].ConqueredCivIcon, g_DominationItemList[numItems].ConqueredCivIconBG, g_DominationItemList[numItems].ConqueredCivIconShadow, false, true);
				g_DominationItemList[numItems].ConqueredCivSmallFrame:SetHide( false );
				g_DominationItemList[numItems].TeamID:SetHide( true );
			...
			for iPlayerLoop, iCountCiv in pairs( aiCapitalOwner ) do
				if iLeadingNumCapitals < iCountCiv then
					iLeadingNumCapitals = iCountCiv;
					iLeadingPlayer = iPlayerLoop;
				end
			end
			if iLeadingNumCapitals > 0 then
				if (iLeadingPlayer == Game.GetActivePlayer()) then
					--print("The current player is winning");
					Controls.DominationLabel:SetText(Locale.ConvertTextKey("TXT_KEY_VP_CONTROL_CIVILIZATION_LEADING", iLeadingNumCapitals, numItems, "TXT_KEY_YOU"));
				else
					...
					Controls.DominationLabel:SetText(Locale.ConvertTextKey("TXT_KEY_VP_CONTROL_CIVILIZATION_LEADING", iLeadingNumCapitals, numItems, pOtherPlayer:GetNickName()));
				    elseif (not Teams[pOtherPlayer:GetTeam()]:IsHasMet(Game.GetActivePlayer())) then
					Controls.DominationLabel:SetText(Locale.ConvertTextKey("TXT_KEY_VP_CONTROL_CIVILIZATION_LEADING", iLeadingNumCapitals, numItems, "TXT_KEY_UNMET_PLAYER"));
				    else
					Controls.DominationLabel:SetText(Locale.ConvertTextKey("TXT_KEY_VP_CONTROL_CIVILIZATION_LEADING", iLeadingNumCapitals, numItems, pOtherPlayer:GetName()));
				    end
				end
...
function PopulateSpaceRace()
	if(PreGame.IsVictory(GameInfo.Victories["VICTORY_SPACE_RACE"].ID))then
		...
		if(g_TechProj) then
			...
			local techProjOffset, techProjAtlas = IconLookup( g_TechProj.PortraitIndex, 128, g_TechProj.IconAtlas );
			Controls.TechProjIcon:SetTexture( techProjAtlas );
			Controls.TechProjIcon:SetTextureOffset( techProjOffset );
			Controls.TechProjIconAnim:SetTexture( techProjAtlas );
			Controls.TechProjIconAnim:SetTextureOffset( techProjOffset );
			if(Teams[iTeam]:GetProjectCount(g_TechProj.ID) == 1) then
				Controls.TechProgress:SetHide(true);
				Controls.TechProject:SetHide(false);
				Controls.BubblesAnim:SetHide(true);
				Controls.TechProjIcon:SetColor(white);
			...
			for i, v in pairs(Teams) do
				if(v:GetProjectCount(g_TechProj.ID) == 1) then
					numStarship = numStarship + 1;
				end
			end
			Controls.SpaceInfo:LocalizeAndSetText("TXT_KEY_VP_DIPLO_PROJECT_PLAYERS_COMPLETE", numStarship, g_TechProj.Description);
		end
...
function PopulateSpaceRaceScreen()
	...
	if(g_TechProj) then
		...
		for i, v in ipairs(sortedPlayerList) do	
			...
			-- Set Tech Project
			local techProjOffset, techProjAtlas = IconLookup( g_TechProj.PortraitIndex, 80, g_TechProj.IconAtlas );
			controlTable.TechProjIcon:SetTexture( techProjAtlas );
			controlTable.TechProjIcon:SetTextureOffset( techProjOffset );
			
			-- Fill in Starship Project
			if(Teams[pTeam]:GetProjectCount(g_TechProj.ID) == 1) then
				controlTable.TechProjIcon:SetColor(white);
...
function CompareSpaceRace(a, b)
	...
	local starshipA = pTeamA:GetProjectCount(g_TechProj.ID) == 1;
	local starshipB = pTeamB:GetProjectCount(g_TechProj.ID) == 1;
...
function GetPreReqs(techType)
	...
	if(not found)then
		table.insert(g_TechPreReqList, techType);
...
--------------------------------------------
if g_TechProj and g_TechProj.TechPrereq then
	GetPreReqs(g_TechProj.TechPrereq);
	print("Total Count of Tech for Victory: " .. #g_TechPreReqList);
end
