...
local g_SecondaryAIM = InstanceManager:new("UnitAction", "UnitActionButton", Controls.SecondaryAStack);
...
local g_EarnedPromotionIM = InstanceManager:new("EarnedPromotionInstance", "UnitPromotionImage",
	Controls.EarnedPromotionStack);

----------------------SP two Rows of Promotions-------------------
local g_EarnedPromotionIM2 = InstanceManager:new("EarnedPromotionInstance", "UnitPromotionImage",
	Controls.EarnedPromotionStack2);
...
function UpdateUnitActions(unit)
	...
	for iAction = 0, #GameInfoActions, 1 do
		...
		if (
			bUnitHasMovesLeft or action.Type == "COMMAND_CANCEL" or action.Type == "COMMAND_STOP_AUTOMATION" or
				action.SubType == ActionSubTypes.ACTIONSUBTYPE_PROMOTION) then
		...
		then
			...
			elseif (action.SubType == ActionSubTypes.ACTIONSUBTYPE_PROMOTION) then
				...
				instance.UnitActionButton:SetOffsetVal((numBuildActions % numberOfButtonsPerRow) * buttonSize + buttonPadding +
					buttonOffsetX + extraXOffset,
					math.floor(numBuildActions / numberOfButtonsPerRow) * buttonSize + buttonPadding + buttonOffsetY);
				numBuildActions = numBuildActions + 1;

			elseif (
				(action.SubType == ActionSubTypes.ACTIONSUBTYPE_BUILD or action.Type == "INTERFACEMODE_ROUTE_TO") and
					hasPromotion == false) then
	...
	for _, action in pairs(addinActions) do
		...
		then
			...
			if ((tonumber(action.OrderPriority) or 200) > 100) then
				...
				instance = g_SecondaryAIM:GetInstance()
				numSecondaryAActions = numSecondaryAActions + 1
			end
	...
	for _, build in pairs(addinBuilds) do
		...
		then
			...
			instance.UnitActionButton:SetAnchor("L,B")
			instance.UnitActionButton:SetOffsetVal((numBuildActions % numberOfButtonsPerRow) * buttonSize + buttonPadding +
				buttonOffsetX, math.floor(numBuildActions / numberOfButtonsPerRow) * buttonSize + buttonPadding + buttonOffsetY)
			numBuildActions = numBuildActions + 1
			...
			then
				...
				if (type(build.Build) == "function") then
					Controls.RecommendedActionButton:RegisterCallback(Mouse.eLClick,
						function() build.Build(build, unit, Mouse.eLClick) end)
					Controls.RecommendedActionButton:RegisterCallback(Mouse.eRClick,
						function() build.Build(build, unit, Mouse.eRClick) end)
				else
	...
	if (numPrimaryActions > 0) then
		...
		Controls.SecondaryButton:SetHide(false);
		Controls.SecondaryButton:SetDisabled(numSecondaryActions == 0 and numSecondaryAActions == 0);
	else
	...
	if (numSecondaryActions == 0 and numSecondaryAActions == 0) then
		Controls.SecondaryStretchy:SetHide(true);
	end
	...
	Controls.SecondaryAStack:CalculateSize();
	Controls.SecondaryAStack:ReprocessAnchoring();

	local stackASize = Controls.SecondaryAStack:GetSize();
	stackSize = Controls.SecondaryStack:GetSize();
	stretchySize = Controls.SecondaryStretchy:GetSize();
	Controls.SecondaryAStack:SetOffsetY(Controls.SecondaryStack:GetOffsetY() + stackSize.y + 2);
	Controls.SecondaryStretchy:SetSizeVal(stretchySize.x, stackASize.y + stackSize.y + 290);
	...
	if numBuildActions > 0 or hasPromotion then
		...
		Controls.WorkerActionPanel:SetSizeVal(stackSize.x,
			math.floor((numBuildActions - 1) / numberOfButtonsPerRow) * buttonSize + buttonPadding + buttonOffsetY + rbOffset +
			workerPanelSizeOffsetY);
	else
		Controls.WorkerActionPanel:SetHide(true);
...
function UpdateUnitStats(unit)
	...
	if unit:GetDomainType() == DomainTypes.DOMAIN_AIR then
		...
		local ignore_terrain = unit:IgnoreTerrainCost() and "*" or " ";
	...
	elseif (unit:GetSpreadsLeft() > 0) then
		strength = math.floor(unit:GetConversionStrength() / GameDefines["RELIGION_MISSIONARY_PRESSURE_MULTIPLIER"]) ..
			" [ICON_PEACE]";
		Controls.UnitStrengthBox:SetHide(false);
		Controls.UnitStatStrength:SetText(strength);
	...
	if (iRangedStrength > 0) then
		...
		local szRangedStrength = iRangedStrength .. "[ICON_RANGE_STRENGTH]" .. unit:Range() .. ignore_LOS;
...
function UpdateUnitHealthBar(unit)
	...
	if damage == 0 then
		...
		local barSize = { x = 9, y = math.floor(153 * healthPercent) };
		...
		Controls.HealthBar:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_UPANEL_SET_HITPOINTS_TT", (MaxDamage - damage),
			MaxDamage));
		--Controls.HealthBar:SetToolTipString(healthPercent.." Hit Points");

		Controls.HealthBar:SetSizeY(157);
...
function TipHandler(control)
	...
	if (action.Type == "COMMAND_UPGRADE") then
		...
		strActionHelp = strActionHelp ..
			Locale.ConvertTextKey("TXT_KEY_UPGRADE_HELP", GameInfo.Units[iUnitType].Description, iGoldToUpgrade);
		...
		if bDisabled then
			...
			for pResource in GameInfo.Resources() do
				...
				if (
					iNumResourceNeededToUpgrade > 0 and
						iNumResourceNeededToUpgrade > pActivePlayer:GetNumResourceAvailable(iResourceLoop)) then
					...
					strResourcesNeeded = strResourcesNeeded ..
						iNumResourceNeededToUpgrade .. " " .. pResource.IconString .. " " .. Locale.ConvertTextKey(pResource.Description);
				end
			end
			...
			if (strResourcesNeeded ~= "") then
				...
				strDisabledString = strDisabledString ..
					Locale.ConvertTextKey("TXT_KEY_UPGRADE_HELP_DISABLED_RESOURCES", strResourcesNeeded);
			end
	...
	if (bDisabled) then
		...
		if (bBuild) then
			...
			if (pBuild.PrereqTech ~= nil) then
				...
				if (iPrereqTech ~= -1 and not pActiveTeam:GetTeamTechs():HasTech(iPrereqTech)) then
					...
					if (pImprovement or pRoute) then
						...
						strDisabledString = strDisabledString .. "[NEWLINE]";
						strDisabledString = strDisabledString ..
							Locale.ConvertTextKey("TXT_KEY_BUILD_BLOCKED_PREREQ_TECH", pPrereqTech.Description, strImpRouteKey);
					end
				end
			end
			...
			if (pImprovement and pImprovement.InAdjacentFriendly) then
				if (pPlot:GetTeam() ~= unit:GetTeam()) then
					if (not pPlot:IsAdjacentTeam(unit:GetTeam(), true)) then
						...
						strDisabledString = strDisabledString .. "[NEWLINE]";
						strDisabledString = strDisabledString ..
							Locale.ConvertTextKey("TXT_KEY_BUILD_BLOCKED_NOT_IN_ADJACENT_TERRITORY", strImpRouteKey);
					end
				end
				...
				if bIsBNW == true then
					...
					if (pImprovement and pImprovement.OnlyCityStateTerritory) then
						...
						if (not bCityStateTerritory) then
							...
							strDisabledString = strDisabledString .. "[NEWLINE]";
							strDisabledString = strDisabledString ..
								Locale.ConvertTextKey("TXT_KEY_BUILD_BLOCKED_NOT_IN_CITY_STATE_TERRITORY", strImpRouteKey);
						end
					end
				end
				if (pPlot:GetTeam() ~= unit:GetTeam()) then
					...
					strDisabledString = strDisabledString .. "[NEWLINE]";
					strDisabledString = strDisabledString ..
						Locale.ConvertTextKey("TXT_KEY_BUILD_BLOCKED_OUTSIDE_TERRITORY", strImpRouteKey);
				end
			end
			...
			if bIsBNW == true then
				if (pImprovement and pImprovement.AdjacentLuxury) then
					...
					if (not bAdjacentLuxury) then
						...
						strDisabledString = strDisabledString .. "[NEWLINE]";
						strDisabledString = strDisabledString ..
							Locale.ConvertTextKey("TXT_KEY_BUILD_BLOCKED_NO_ADJACENT_LUXURY", strImpRouteKey);
					end
				end
				...
				if (pImprovement and pImprovement.NoTwoAdjacent) then
					...
					if (bTwoAdjacent) then
						...
						strDisabledString = strDisabledString .. "[NEWLINE]";
						strDisabledString = strDisabledString ..
							Locale.ConvertTextKey("TXT_KEY_BUILD_BLOCKED_CANNOT_BE_ADJACENT", strImpRouteKey);
					end
				end
			...
			if (pActivePlayer:IsBuildBlockedByFeature(iBuildID, iFeature)) then
				...
				strDisabledString = strDisabledString .. "[NEWLINE]";
				strDisabledString = strDisabledString ..
					Locale.ConvertTextKey("TXT_KEY_BUILD_BLOCKED_BY_FEATURE", pFeatureTech.Description, pFeature.Description);
			end
			...
			elseif (action.Type == "MISSION_CULTURE_BOMB" and pActivePlayer:GetCultureBombTimer() > 0) then
				...
				strDisabledString = strDisabledString ..
					Locale.ConvertTextKey("TXT_KEY_MISSION_CULTURE_BOMB_DISABLED_COOLDOWN", pActivePlayer:GetCultureBombTimer());
			...
			if (iResourceID ~= -1) then
				if (pPlot:IsResourceConnectedByImprovement(iImprovement)) then
					if (Game.GetResourceUsageType(iResourceID) ~= ResourceUsageTypes.RESOURCEUSAGE_BONUS) then
						...
						strBuildResourceConnectionString = strBuildResourceConnectionString .. "[NEWLINE]";
						strBuildResourceConnectionString = strBuildResourceConnectionString ..
							Locale.ConvertTextKey("TXT_KEY_BUILD_CONNECTS_RESOURCE", pResource.IconString, strResourceString);
		...
		if (pFeature) then
			...
			if (bFeatureRemoved) then
				...
				strClearFeatureString = strClearFeatureString ..
					Locale.ConvertTextKey("TXT_KEY_BUILD_FEATURE_CLEARED", pFeature.Description);
			end

			local iFeatureProduction = pPlot:GetFeatureProduction(iBuildID, iActiveTeam);
			if (iFeatureProduction > 0) then
				strClearFeatureString = strClearFeatureString ..
					Locale.ConvertTextKey("TXT_KEY_BUILD_FEATURE_PRODUCTION", iFeatureProduction);
