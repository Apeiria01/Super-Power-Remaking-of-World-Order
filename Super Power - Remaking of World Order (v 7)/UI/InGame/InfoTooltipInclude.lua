...
-- modified by bc1 from brave new world code
...
--n	Mechanized = "",				-- art only ?
	...
	ProhibitsSpread = L"TXT_KEY_PROHIBITS_SPREAD",				-- TODO (inquisitors)
...
--	Unhappiness = L"2": %i[ICON_HAPPINESS_3]",	
...
-- todo determine sea resources
	...
	if GameInfo.Building_FreeUnits_Truly then
		...
		local freeUnit = GameInfo.Units[ row.UnitType ]
		freeUnit = freeUnit and GetCivUnit( activeCivilizationType, freeUnit.Class )
		tips:insertIf( freeUnit and (row.NumUnits or 0)>0 and ( row.NumUnits > 1 and row.NumUnits.." " or "" ) ..L"TXT_KEY_FREE".." ("..L"TXT_KEY_TRULY"..") "..UnitColor( L( freeUnit.Description ) ) )
	    end
	end
	...
	if civ5_mode then
		...
		if gk_mode then
			...
			for row in GameInfo.BuildingClasses() do
				...
				or(civ5_mode and GameInfo.Building_BuildingClassHappiness{ BuildingType = buildingType, BuildingClassType = row.Type }())
				then
					table.insert( buildingClassTypes, row.Type )
				end
			end
...
--]]
	...
	local function getDealTurnsRemaining( itemID )
		...
		if bnw_mode and itemID == TradeableItems.TRADE_ITEM_DECLARATION_OF_FRIENDSHIP then -- DoF or Denounced special kinky case
