...
local colorYellow = {x=1, y=1, z=0, w=1 };
...
function UpdateSmallButtonAlpha( buttonAdded, image, shadow, button, textureSheet, textureOffset, category, localizedText, buttonId )
	...
	button:SetOffsetVal( (buttonAdded % numberOfButtonsPerRow) * buttonSize + buttonPadding, math.floor(buttonAdded / numberOfButtonsPerRow) * buttonSize + buttonPadding - 3 );
	button:SetToolTipString( localizedText );
end
...
function SearchForEliteUnitOwner( unitID, pProject )
	...
	if (Game ~= nil) then
		for playerID = 0, GameDefines.MAX_CIV_PLAYERS - 1 do
			...
			if pPlayer ~= nil and pPlayer:IsEverAlive()
			and Teams[Players[Game.GetActivePlayer()]:GetTeam()]:IsHasMet(pPlayer:GetTeam())
			and Teams[pPlayer:GetTeam()]:GetProjectCount( pProject.ID ) > 0
			then
				oPlayer = pPlayer;
			end
		end
	...
	if unitID ~= -1 then
		...
		if thisUnitFlagAtlasInstance then
			...
			if (thisUnit.ProjectPrereq) then
				...
				thisUnitFlagAtlasInstance.UnitFlagAtlasImage:SetColor( colorYellow );
			    else
			...
			if (Game ~= nil and not bColorChanged) then
				local pPlayer = nil;
				...
				if     (thisUnit.ProjectPrereq) then
					...
					local iMaxInstances = prereqProject.MaxGlobalInstances;
					if (iMaxInstances == 1) then
						pPlayer = SearchForEliteUnitOwner( unitID, prereqProject );
					else
						if (aPlayer and Teams[aPlayer:GetTeam()]:GetProjectCount( prereqProject.ID ) > 0) then
							pPlayer = aPlayer;
						end
					end
				elseif (thisUnit.ReligiousStrength > 0) then
					if     aPlayer:HasCreatedReligion() then
						pPlayer = aPlayer;
					elseif aPlayer:GetCapitalCity() ~= nil then
					    local eReligion = aPlayer:GetCapitalCity():GetReligiousMajority();
					    if eReligion > ReligionTypes.RELIGION_PANTHEON then
						pPlayer = Players[Game.GetFounder(eReligion, -1)];
					    end
					end
				else
					pPlayer = aPlayer;
...
function SelectBuildingOrWonderArticle( buildingID )
	if buildingID ~= -1 then
		...
		if thisBuilding.Mountain then
			if (not bRestriction) then
				bRestriction = true;
				strRestriction = Locale.ConvertTextKey( "TXT_KEY_CIVILOPEDIA_CONSTRUCTION_MOUNTAIN" );
			else
				strRestriction = strRestriction .. "[NEWLINE]" .. Locale.ConvertTextKey( "TXT_KEY_CIVILOPEDIA_CONSTRUCTION_MOUNTAIN" );
			end
		end
			...
			if thisCiv and thisCiv.Type ~= "CIVILIZATION_MINOR" and thisCiv.Type ~= "CIVILIZATION_BARBARIAN" then
				...
				if tagString then
					...
					for i = 1, iCount-1, 1 do
						if stringTable[flavorList[i]] ~= nil then
							if (not bNewBlock) then
								bodyString = bodyString .. "[NEWLINE]";
							else
								bNewBlock = false;
							...
							bodyString = bodyString .. stringTable[flavorList[i]].strTag;
						end
					end
