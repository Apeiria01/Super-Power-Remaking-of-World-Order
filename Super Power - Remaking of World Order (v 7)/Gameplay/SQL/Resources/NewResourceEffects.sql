...
('FORMULA_ELECTRICITY_BONUS', 'local num, cityCount, numTech = ... 	local lowerLimit = math.max(-75, -math.max(0, numTech - 50) * 5) local upperLimit = math.min(50, math.max(0, numTech - 50) * 5) if num < 0 then return math.max(lowerLimit, cityCount * num) else if num >= 25 then return math.min(upperLimit, math.floor(num / cityCount)) else return 0 end end');
