...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		-- Bonus
		wheat_ID = -1, cow_ID = -1, deer_ID = -1, banana_ID = -1, fish_ID = -1, sheep_ID = -1, stone_ID = -1, bison_ID = -1,
		-- Rush
		iron_ID = -1, horse_ID = -1, coal_ID = -1, oil_ID = -1, aluminum_ID = -1, uranium_ID = -1,
		-- Luxuries
		whale_ID = -1, pearls_ID = -1, ivory_ID = -1, fur_ID = -1, silk_ID = -1,
		dye_ID = -1, spices_ID = -1, sugar_ID = -1, cotton_ID = -1, wine_ID = -1,
		incense_ID = -1, gold_ID = -1, silver_ID = -1, gems_ID = -1, marble_ID = -1,
		-- Expansion Luxuries
		copper_ID = -1, salt_ID = -1, citrus_ID = -1, truffles_ID = -1, crab_ID = -1, cocoa_ID = -1,
		-- Mod Luxuries
...
function AssignStartingPlots:GenerateRegions(args)
	...
	-- MOD.CaptainCWB: If using a MOD to make NumCivs more than 22, set 22+ Civs to CityStates
	-- MOD.CaptainCWB: Start
	if self.iNumCivs > 22 then
		self.iNumCityStates = self.iNumCityStates + self.iNumCivs - 22;
		self.iNumCivs = 22;
	end
	-- MOD.CaptainCWB: End
...
function AssignStartingPlots:FindStart(region_number)
	...
	if iNumFallbacks > 0 then
		...
		if GameInfo.Features[forcePlot:GetFeatureType()] == nil or not GameInfo.Features[forcePlot:GetFeatureType()].NaturalWonder then
...
function AssignStartingPlots:AttemptToPlaceBonusResourceAtPlot(x, y, bAllowOasis)
	...
	elseif plotType == PlotTypes.PLOT_HILLS and featureType == FeatureTypes.NO_FEATURE then
		...
		return true, false
	elseif plotType == PlotTypes.PLOT_LAND  and featureType == FeatureTypes.NO_FEATURE then
...
function AssignStartingPlots:AttemptToPlaceHillsAtPlot(x, y)
	...
	if GameInfo.Features[plot:GetFeatureType()] == nil or not GameInfo.Features[plot:GetFeatureType()].NaturalWonder then
	...
	if bDisableStartBias == 1 then
		...
		for region_number, player_ID in ipairs(playerListShuffled) do
			...
			if player:GetStartingPlot() then
				self.startingPlots[region_number][1] = player:GetStartingPlot():GetX();
				self.startingPlots[region_number][2] = player:GetStartingPlot():GetY();
			else
		...
		if iNumCoastalCivs - iNumUnassignableCoastStarts > 0 then
			...
			for loop, playerNum in ipairs(shuffled_coastal_civs) do
				...
				if loop <= iNumRegionsWithCoastalStart then
					...
					local player = Players[playerNum];
					if player:GetStartingPlot() then
						self.startingPlots[choose_this_region][1] = player:GetStartingPlot():GetX();
						self.startingPlots[choose_this_region][2] = player:GetStartingPlot():GetY();
					else
...
function AssignStartingPlots:PlaceCityStateInRegion(city_state_number, region_number)
	...
	if placed_city_state == true then
		...
		local city_state_ID = city_state_number + 22 - 1;
		local cityState = Players[city_state_ID];
		if cityState:GetStartingPlot() then
			x = cityState:GetStartingPlot():GetX();
			y = cityState:GetStartingPlot():GetY();
		end
		...
		if cs_number <= self.iNumCityStates then -- Make sure it's an active city state before processing.
			if region_number == -1 and iUninhabitedCandidatePlots > 0 then -- Assigned to areas outside of Regions.
				...
				if success == true then
					local city_state_ID = cs_number + 22 - 1;
					local cityState = Players[city_state_ID];
					if cityState:GetStartingPlot() then
						cs_x = cityState:GetStartingPlot():GetX();
						cs_y = cityState:GetStartingPlot():GetY();
					end
	...
	for place_resource = 1, iNumResourcesToPlace do
			...
			for index_to_check = current_index, iNumTotalPlots do
				...
				if impact_table_number == 1 then
					if self.strategicData[plotIndex] == 0 then
						...
						if res_plot:GetResourceType(-1) == -1
						and (GameInfo.Features[res_plot:GetFeatureType()] == nil or not GameInfo.Features[res_plot:GetFeatureType()].NaturalWonder)
						then -- Placing this strategic resource in this plot.
				...
				elseif impact_table_number == 2 then
					if self.luxuryData[plotIndex] == 0 then
						...
						if res_plot:GetResourceType(-1) == -1
						and (GameInfo.Features[res_plot:GetFeatureType()] == nil or not GameInfo.Features[res_plot:GetFeatureType()].NaturalWonder)
						then -- Placing this luxury resource in this plot.
							local res_addition = 0;
							if res_range[use_this_res_index] ~= -1 then
								res_addition = Map.Rand(res_range[use_this_res_index], "Resource Radius - Place Resource LUA");
							end
				...
				elseif impact_table_number == 3 then
					if self.bonusData[plotIndex] == 0 then
						...
						if res_plot:GetResourceType(-1) == -1
						and (GameInfo.Features[res_plot:GetFeatureType()] == nil or not GameInfo.Features[res_plot:GetFeatureType()].NaturalWonder)
						then -- Placing this bonus resource in this plot.
							local res_addition = 0;
							if res_range[use_this_res_index] ~= -1 then
								res_addition = Map.Rand(res_range[use_this_res_index], "Resource Radius - Place Resource LUA");
							end
		...
		if avoid_ripples == false then -- Completed first pass through plot_list, so use backup method.
			...
			if best_plot ~= nil then
				...
				if (GameInfo.Features[res_plot:GetFeatureType()] == nil or not GameInfo.Features[res_plot:GetFeatureType()].NaturalWonder) then
...
function AssignStartingPlots:PlaceSpecificNumberOfResources(resource_ID, quantity, amount,
	...
	if plot_list == nil or resource_ID == -1 then
			...
			if resource_ID == self.marble_ID then	-- MOD.Barathor: Temporary Fix: Marble's additional impact and ripple values on the marble layer were originally disregarded.
				if bCheckImpact == false or impact_table[plotIndex] == 0 then
					if self.marbleData[plotIndex] == 0 then
						...
						if res_plot:GetResourceType(-1) == -1
						and (GameInfo.Features[res_plot:GetFeatureType()] == nil or not GameInfo.Features[res_plot:GetFeatureType()].NaturalWonder)
						then -- Placing this resource in this plot.
...
function AssignStartingPlots:AssignLuxuryToRegion(region_number)
	...
	for index, resource_options in ipairs(luxury_candidates) do
		...
		if res_ID ~= -1 and self.luxury_assignment_count[res_ID] < split_cap then -- This type still eligible.
			...
			if self.iNumTypesAssignedToRegions < self.iNumMaxAllowedForRegions or test == true then -- Not a new type that would exceed number of allowed types, so continue.
				...
				if res_ID == self.whale_ID or res_ID == self.pearls_ID or res_ID == self.crab_ID or res_ID == self.coral_ID then
					...
					if self.startLocationConditions and self.startLocationConditions[region_number][1] == true then -- This region's start is along an ocean, so water-based luxuries are allowed.
						-- MOD.Barathor: Start
	...
	if iNumAvailableTypes == 0 then
		for index, resource_options in ipairs(self.luxury_fallback_weights) do
			local res_ID = resource_options[1];
			if res_ID ~= -1 and self.luxury_assignment_count[res_ID] < 3 then -- This type still eligible.
				local test = TestMembership(self.resourceIDs_assigned_to_regions, res_ID)
...
function AssignStartingPlots:AssignLuxuryRoles()
	...
	for index, resource_options in ipairs(self.luxury_city_state_weights) do
		local res_ID = resource_options[1];
		local test = TestMembership(self.resourceIDs_assigned_to_regions, res_ID)
		if res_ID ~= -1 and test == false then
			table.insert(resource_IDs, res_ID);
	...
	for index, resource_options in ipairs(self.luxury_fallback_weights) do
		...
		if res_ID ~= -1 and test1 == false and test2 == false then
	...
	for index, resource_options in ipairs(self.luxury_fallback_weights) do
		...
		if res_ID ~= -1 and test1 == false and test2 == false and test3 == false then
...
function AssignStartingPlots:GetIndicesForLuxuryType(resource_ID)
	...
	elseif resource_ID == self.whale_ID then
		primary = 1;
...
function AssignStartingPlots:FixResourceGraphics()
	...
	for y = 0, iH - 1 do
		for x = 0, iW - 1 do
			...
			-- Invalid Resources do nothing
			if     res_ID == -1 then
			-- Mined/Quarried Resources
			elseif res_ID == self.marble_ID
			or     res_ID == self.gold_ID
			or     res_ID == self.silver_ID
			or     res_ID == self.copper_ID
			or     res_ID == self.gems_ID
			or     res_ID == self.salt_ID
			or     res_ID == self.lapis_ID
			or     res_ID == self.jade_ID
			or     res_ID == self.amber_ID
			then
			...
			-- Tree Resources
			elseif res_ID == self.cocoa_ID
			or     res_ID == self.citrus_ID
			or     res_ID == self.spices_ID
			or     res_ID == self.sugar_ID
			or     res_ID == self.truffles_ID
			or     res_ID == self.silk_ID
			or     res_ID == self.dye_ID
			or     res_ID == self.fur_ID
			or     res_ID == self.deer_ID
			then
			...
			-- Open Land Resources
			elseif res_ID == self.incense_ID
			or     res_ID == self.ivory_ID
			or     res_ID == self.wine_ID
			or     res_ID == self.olives_ID
			or     res_ID == self.coffee_ID
			or     res_ID == self.tobacco_ID
			or     res_ID == self.tea_ID
			or     res_ID == self.perfume_ID
			or     res_ID == self.cotton_ID
			then
