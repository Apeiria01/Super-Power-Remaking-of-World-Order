...
function DoInternationalImmigration(MoveOutPlayerID, MoveInPlayerID)
    ...
    for pCity in MoveInPlayer:Cities() do
        ...
        if cityPop > 0 and cityPop < 80 and not pCity:IsPuppet() and
            ...
            not pCity:IsHasBuilding(GameInfoTypes["BUILDING_NO_UTILITY_WARNING"]) and
...
function CitySetSize(city, player, cityPop)
    ...
    if GameInfo.Leader_Traits {
        LeaderType = GameInfo.Leaders[player:GetLeaderType()].Type,
        TraitType = "TRAIT_DIPLOMACY_GREAT_PEOPLE"
    }() and
        (GameInfo.Traits["TRAIT_DIPLOMACY_GREAT_PEOPLE"].PrereqPolicy == nil or
            (GameInfo.Traits["TRAIT_DIPLOMACY_GREAT_PEOPLE"].PrereqPolicy and
                player:HasPolicy(
                    GameInfoTypes[GameInfo.Traits["TRAIT_DIPLOMACY_GREAT_PEOPLE"]
                        .PrereqPolicy]))) and cityPop >= 6 then
        if cityPop >= 6 then
            city:SetNumRealBuilding(
                GameInfoTypes["BUILDING_TB_DIPLOMACY_GREAT_PEOPLE"], 1);
        else
            city:SetNumRealBuilding(
                GameInfoTypes["BUILDING_TB_DIPLOMACY_GREAT_PEOPLE"], 0);
        end
    end
    ...
    elseif cityPop >= 60 then
        ...
        if city:IsHasBuilding(GameInfo.Buildings["BUILDING_MEDICAL_LAB"].ID) or
            ...
            city:SetNumRealBuilding(
                GameInfoTypes["BUILDING_NO_UTILITY_WARNING"], 0)
        else
            city:SetNumRealBuilding(
                GameInfoTypes["BUILDING_NO_UTILITY_WARNING"], 1) ----Stop population growth!
            city:SetFood(0)
            print(
                "This city doesn't have Utility building so the population growth has stopped!")
        end
    ...
    elseif cityPop >= 15 then
        ...
        if city:IsHasBuilding(GameInfo.Buildings["BUILDING_AQUEDUCT"].ID) or
            ...
            city:IsHasBuilding(
                GameInfo.Buildings["BUILDING_BASILICA_CISTERN"].ID) or
            city:IsHasBuilding(
                GameInfo.Buildings["BUILDING_TAP_WATER_SUPPLY"].ID) then --------Check if city has utility building?
            city:SetNumRealBuilding(
                GameInfoTypes["BUILDING_NO_UTILITY_WARNING"], 0)
        else
            city:SetNumRealBuilding(
    ...
    if ConsumerRes <= 4 then
        ...
        if not city:IsResistance() and not city:IsNoAutoAssignSpecialists() and
            not city:IsRazing() and city:GetYieldRate(YieldTypes.YIELD_FOOD) >=
            2 then
            for building in GameInfo.Buildings() do
                if building.SpecialistType == "SPECIALIST_MERCHANT" then
                    local buildingID = building.ID
                    if city:IsHasBuilding(buildingID) then
                        if city:GetNumSpecialistsAllowedByBuilding(buildingID) >
                            0 and city:GetNumSpecialistsInBuilding(buildingID) <
                            2 then
                            city:DoTask(TaskTypes.TASK_ADD_SPECIALIST,
                                        GameInfo.Specialists.SPECIALIST_MERCHANT
                                            .ID, buildingID)
                            print(
                                "Auto add a merchant to fill the consumer goods' gap!")
                        end
                    end
                end
            end
...
function RemoveRealBuildingsiffree(iPlayer, iCity, iBuilding, bGold, bFaith)
    ...
    if pPlayer == nil or not pPlayer:IsAlive() or pPlayer:IsMinorCiv() or
        pPlayer:IsBarbarian() or pPlayer:GetCityByID(iCity) == nil then
        return
    end
    print("City constructed: " .. GameInfo.Buildings[iBuilding].Type)
    local FBC = GameInfo.Buildings[iBuilding].FreeBuilding
    if FBC == nil or
        GameInfo.Buildings[GameInfo.BuildingClasses[FBC].DefaultBuilding].Cost <
        1 or
        GameInfo.BuildingClasses[GameInfo.Buildings[iBuilding].BuildingClass]
            .MaxGlobalInstances > 0 then return; end
    ...
    local overrideBuilding = GameInfo.Civilization_BuildingClassOverrides {
        BuildingClassType = FBC,
...
function RemoveRealBuildingswhencapturingFreewonders(oldPlayerID, bCapital, iX,
                                                     iY, newPlayerID, conquest,
                                                     conquest2)
    if Players[newPlayerID] == nil or Map.GetPlot(iX, iY) == nil or
        not Map.GetPlot(iX, iY):IsCity() then return; end
    local pPlayer = Players[newPlayerID];
    local pCity = Map.GetPlot(iX, iY):GetPlotCity();
    for pBuilding in GameInfo.Buildings() do
        if pBuilding.FreeBuilding ~= nil and not pBuilding.NeverCapture and
            GameInfo.BuildingClasses[pBuilding.BuildingClass].MaxGlobalInstances <=
            0 then
            local FBC = pBuilding.FreeBuilding
            print("Free BuildingClass is" .. FBC)
            if GameInfo.Buildings[GameInfo.BuildingClasses[FBC].DefaultBuilding]
                .Cost >= 0 and pCity:GetNumBuilding(pBuilding.ID) > 0 then
                print("Free Building city wonder provides: " .. FBC)
                pCity:SetNumRealBuilding(pBuilding.ID, 0)
                local FreeBuildingType =
                    GameInfo.BuildingClasses[FBC].DefaultBuilding
                local overrideBuilding =
                ...
                for city in pPlayer:Cities() do
                    city:SetNumRealBuilding(GameInfoTypes[FreeBuildingType], 0)
                    print("Real building removed!")
                    local numbuild = city:GetNumBuilding(
                                         GameInfoTypes[FreeBuildingType])
                    print("Num of building: " .. numbuild)
                    local numrb = city:GetNumRealBuilding(
                                      GameInfoTypes[FreeBuildingType])
                    print("Num of real building: " .. numrb)
                end
