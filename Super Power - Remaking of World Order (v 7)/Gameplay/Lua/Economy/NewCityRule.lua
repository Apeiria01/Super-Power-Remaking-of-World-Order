...
function PlayerEditCity()------------------------This will trigger when player edit the specialists slots inside their cities.
	...
	function SetCitySpecialistResources(city)
		...
		if CityEngineerCount > 0 then
			...
			if Teams[player:GetTeam()]:IsHasTech(GameInfo.Technologies["TECH_ELECTRICITY"].ID)
			then
				city:SetNumRealBuilding(GameInfoTypes["BUILDING_SPECIALISTS_ELECTRICITY"],CityEngineerCount)
			end
		end
